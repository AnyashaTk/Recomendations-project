...
class Preprocessing:
    def __init__(self, users: pd.DataFrame, items: pd.DataFrame,
                 interactions: pd.DataFrame):
        train = self.__preprocess(users, items, interactions)
        users, items, user_features = self.__prepare_features(users, items,
                                                              train)
    ...
    def __preprocess(users, items, interactions) -> pd.DataFrame:
        ...
        train = interactions[
            interactions[Columns.Datetime] < max_date - pd.Timedelta(
                days=7)].copy()
